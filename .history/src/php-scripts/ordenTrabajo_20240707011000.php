...
$desc = $argv[10];
$obs = $argv[11];
$items = json_decode($argv[12], true);
...
if ($return_var === 0) {
    ...
    $pdf_file_name = 'ordenTrabajo.pdf';
    if (file_exists($pdf_file_name)) {
        header('Content-Description: File Transfer');
        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="' . basename($pdf_file_name) . '"');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
        header('Content-Length: ' . filesize($pdf_file_name));
