...
export const abonarSolicitud = async (req, res) => {
  try {
    ...
    // Verificar si folioExterno ya está en uso por otra solicitud
    const existeFolioExterno = await Solicitud.exists({ folioExterno });
    if (existeFolioExterno) {
      // Si existe, no modificar el campo folioExterno
      console.log(
        `El folio externo '${folioExterno}' ya está en uso por otra solicitud.`
      );
    } else {
