...
export const abonarSolicitud = async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      if (suministro) {
        const nuevaCantidadAcumulada = suministro.cantidadAcumulada + parseInt(item.cantidadEntregada);
        ...
        return res.status(404).json({ error: `Suministro ${item._id} no encontrado en la solicitud.` });
      }
    }

    await solicitudExistente.save();
    console.log("Solicitud actualizada exitosamente");
    return res.json(solicitudExistente);
  } catch (error) {
