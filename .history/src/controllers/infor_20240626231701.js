...
export const crearInforme = async (req, res) => {
  try {
    ...
    if (req.files?.imagen) {
      ...
      //nustra funcion impotada para subir archivos a cloudinary
...
export const eliminarInforme = async (req, res) => {
  try {
    ...
    const informe = await InformeTecnico.findByIdAndDelete(id);

    if (!informe) {
      return res.status(404).json({ mensaje: "Informe técnico no encontrado" });
    }

    //importamos nuestra funcion de elimar archivos de cloudynary
    await deleteImage(informe.public_id)
