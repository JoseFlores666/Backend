...
export const abonarSolicitud = async (req, res) => {
  try {
    ...
    solicitudExistente.estado = solicitudExistente.estado.trim();
    console.log(`Estado actual: '${solicitudExistente.estado}'`);

    if (!["Pendiente", "Asignada", "Diagnosticada", "Atendida", "Rechazada"].includes(solicitudExistente.estado)) {
      return res.status(400).json({ error: `Estado inválido: ${solicitudExistente.estado}` });
    }
