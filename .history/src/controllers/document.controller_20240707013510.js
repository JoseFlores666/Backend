...
export const generateOrdenTrabajo = (req, res) => {
  ...
  const args = [
    ...
    desc,
    obs,
    jsonItems,// Convertir items a JSON para pasar como argumento
  ];

  // Escapar argumentos para evitar problemas con caracteres especiales
  const escapedArgs = shellEscape(args);
  const command = `php ${phpScriptPath} ${escapedArgs}`;
