...
export const obtenerProyectoYActividades = async (req, res) => {
  try {
    const { nombre } = req.params; // Cambia "id" por "nombre" para buscar por nombre
    const proyecto = await Proyecto.findOne({ nombre }).populate('actividades'); // Busca el proyecto por nombre y popula las actividades
