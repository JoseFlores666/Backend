...
export const crearFirma = async (req, res) => {
    try {
        ...
        const nuevaFirma = new Firma({ solicitud, revision, validacion, autorizacion });
...
export const 2obtenerFirmas = async (req, res) => {
    try {
...
export const editarFirma = async (req, res) => {
    try {
        const { id } = req.params;
        const { solicitud, revision, validacion, autorizacion } = req.body;
        const firmaEditada = await Firma.findByIdAndUpdate(id, { solicitud, revision, validacion, autorizacion }, { new: true });
        if (!firmaEditada) {
            return res.status(404).json({ mensaje: 'Firma no encontrada' });
        }
