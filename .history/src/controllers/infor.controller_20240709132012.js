...
export const crearInforme = async (req, res) => {
  try {
    ...
    // Verificar qué datos trae req.files
    ...
    if (req.files) {
      // Obtener todas las claves de los archivos en req.files
      const fileKeys = Object.keys(req.files);

      for (const key of fileKeys) {
        const file = req.files[key];

        // Verifica qué datos trae file
        console.log(file);

        // Sube la imagen a Cloudinary usando el campo correcto
