...
const API_KEY = "jjfe_08@hotmail.com_FBw0jSp7ktM4AiIG43kyGHD92WK0zA5U483PHk62eH64Wd0zTBdpYGmni5id4xn2";
...
function convertDocToPdf(apiKey, uploadedFileUrl, destinationFile) {
    return new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    https.get(json.url, response2 => {
                        response2.pipe(file).on("close", () => {
                            ...
                            resolve(); // Resuelve la promesa cuando el archivo se ha guardado correctamente
                        });
                    });
                } catch (error) {
                    reject(new SyntaxError("Failed to parse JSON response: " + data));
        ...
        postRequest.on('error', (err) => {
            reject(err); // Rechaza la promesa si hay algún error en la solicitud
        });
