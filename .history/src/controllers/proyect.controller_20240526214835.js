...
export const trae_Proyect_Act_Nombre = async (req, res) => {
  try {
    ...
    const decodedNombre = decodeURIComponent(nombre); // Decodificar el nombre del proyecto
    console.log(decodedNombre);
    const proyecto = await Proyecto.findOne({ nombre: decodedNombre }).populate('actividades');
