...
const executeWithRetry = async (func, args, retries = 3) => {
...
(async () => {
  try {
    const [uploadUrl, uploadedFileUrl] = await executeWithRetry(getPresignedUrl, [API_KEY, SourceFile]);
    await executeWithRetry(uploadFile, [SourceFile, uploadUrl]);
    await executeWithRetry(convertDocToPdf, [API_KEY, uploadedFileUrl, DestinationFile]);
  } catch (error) {
