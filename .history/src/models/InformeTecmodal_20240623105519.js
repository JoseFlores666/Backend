...
import FolioCounter from "./.modal.js";
...
// Middleware pre-save para generar el folio
informeTecnicSchema.pre('save', async function(next) {
  if (!this.folio) {
    ...
    if (!folioCounter) {
      ...
      folioCounter = new FolioCounter({ counterInforme: 0, monthInforme: currentMonth, yearInforme: currentYear });
    }

    // Si el mes ha cambiado, reiniciar el contador de informes a 0
    if (folioCounter.monthInforme !== currentMonth || folioCounter.yearInforme !== currentYear) {
    ...
    this.folio = `${currentYear}/${currentMonth}/ Num:${folioCounter.counterInforme.toString().padStart(5, '0')}`;
  }
  next();
});
