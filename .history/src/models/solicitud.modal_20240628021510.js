...
solicitudSchema.pre('save', async function(next) {
  if (!this.folio) {
    ...
    if (!folioCounter) {
      ...
      folioCounter = new FolioCounter({ counter: 0, month: currentMonth, year: currentYear });
    }

    // Si el mes ha cambiado, reiniciar el contador a 0
    if (folioCounter.month !== currentMonth || folioCounter.year !== currentYear) {
    ...
    // Generar el folio
    this.folio = `${currentYear}/${currentMonth}/${folioCounter.counter.toString().padStart(3, '0')}`;
  }
  next();
});
