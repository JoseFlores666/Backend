...
export function SoliProvider({ children }) {
  ...
  const getIdsProyect = async () => {
    try {
      ...
      setIds(res.data);//id de todos los proyectos
    } catch (error) {
  ...
  const traeProyectosActividadesNombrep = async (nombre) => {
    try {
      console.log(nombre)
      const res = await trae_Proyect_Act_Nombre(nombre);
      if(res){console.log("se pudo")}else{
        console.log("No se pudo Dx")
      }
