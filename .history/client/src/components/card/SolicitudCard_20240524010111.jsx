...
export function SolicitudCard({ solicitud }) {
  ...
  const handleDelete = async () => {
    if (isDeleting || !solicitud._id) return; // Evita llamadas repetidas o sin ID
    setIsDeleting(true);
    await deleteSoli(solicitud._id);
  };

  const handleEdit = () => {
    if (isEditing || !solicitud._id) return; // Evita llamadas repetidas o sin ID
    setIsEditing(true);
    console.log(`/soli/${solicitud._id}`)
