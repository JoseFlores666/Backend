...
export function SolicitudCard({ soli }) {
  ...
  useEffect(() => {
    if (deleted) {
      // Actualizar el estado de deleted a false después de 2 segundos para simular la recarga del componente
      const timeout = setTimeout(() => {
        setDeleted(false);
      }, 2000);

      // Limpiar el timeout al desmontar el componente para evitar fugas de memoria
