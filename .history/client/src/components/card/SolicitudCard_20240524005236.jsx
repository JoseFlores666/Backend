...
export function SolicitudCard({ solicitud }) {
    ...
    const [deleteRequested, setDeleteRequested] = useState(false); // Nuevo estado para eliminar
    const [editRequested, setEditRequested] = useState(false); // Nuevo estado para editar
  
    useEffect(() => {
      // Restablecer el estado cuando la solicitud cambia
      setDeleted(false);
    ...
    const handleDelete = async () => {
      if (deleteRequested || !solicitud._id) return; // Evita llamadas repetidas o sin ID
      setDeleteRequested(true);
      setIsDeleting(true);
      await deleteSoli(solicitud._id);
      setTimeout(() => {
        setDeleted(true);
      }, 1000);
    };
    ...
    const handleEdit = () => {
      if (editRequested || !solicitud._id) return; // Evita llamadas repetidas o sin ID
