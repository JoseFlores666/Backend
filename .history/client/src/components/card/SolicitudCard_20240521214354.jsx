...
export function SolicitudCard({ solicitud }) {
  ...
  const handleEdit = () => {
    // Redirigir a la página de edición con el ID de la solicitud
    navigate(`/editar-solicitud/${solicitud._id}`);
  };
      ...
      <Card className={`p-4 mb-4 rounded-lg bg-white ${isDeleting ? 'opacity-50' : ''} shadow-lg shadow-white/40`}>
        ...
        <footer className="flex justify-between items-center">
          <div className="flex gap-4 mb-4">
            <Button onClick={() => handleDelete(solicitud._id)}>Eliminar</Button>
            <ButtonLink to={`/soliPager/${solicitud._id}`}>Editar</ButtonLink>
          </div>
