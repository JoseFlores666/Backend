...
export const RegisterSolicitudPage = () => {
  ...
  const [actividadNombre, setActividadNombre] = useState(""); // E
  const {
    crearmySoli,
    getIdsProyect,
  ...
  const miFuncionPrincipal = async () => {
    ...
    try {
      ...
      setProyectoNombre(proyect.nombre); // Actualiza el nombre del proyecto
      setActividadNombre(unProyectAct.nombre); // Actualiza el nombre de la actividad
      return Promise.resolve(); // Resuelve la promesa una vez que se han actualizado los nombres
    } catch (error) {
  ...
  function generarPDF() {
    ...
    miFuncionPrincipal().then(() => {
      ...
      itemsChunks
        ...
        .catch((error) => {
          console.error(
            "Error al obtener nombres de proyecto y actividad:",
            error
          );
        });
    });
