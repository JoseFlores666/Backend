...
export const RegisterSolicitudPage = () => {
  ...
  const [projectsLoaded, setProjectsLoaded] = useState(false);
  const [activitiesLoaded, setActivitiesLoaded] = useState(false); = useState(false);
  ...
  //   try {
  //    await getIdsProyectYActIndividual(idProyecto, idActividad);
  ...
  useEffect(() => {
    ...
    if (fetchActivitiesFlag || fetchPDFFlag) {
      fetchIndividualProyectAndActivity();
      setFetchActivitiesFlag(false);
      setFetchPDFFlag(false);
    }
  }, [fetchActivitiesFlag, fetchPDFFlag, datos.proyecto, datos.actividad, getIdsProyectYActIndividual]);
