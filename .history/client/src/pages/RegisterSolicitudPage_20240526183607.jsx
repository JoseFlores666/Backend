...
export const RegisterSolicitudPage = () => {
  ...
  const handleProyectoChange = async (value) => {
    ...
    setActividadId(""); // Reiniciar el ID de la actividad al cambiar de proyecto
  };

  const handleActividadChange = (value) => {
    setActividad(value); // Actualizar el nombre de la actividad seleccionada
    const actividad = idsAct.find((act) => act._id === value);
    if (actividad) {
      setActividadId(actividad._id); // Actualizar el ID de la actividad seleccionada
    }
  };
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      if (selectedProyecto && !activitiesLoaded) {
        try {
          await getIdsProyectYAct(selectedProyecto, actividadId);
    ...
    fetchActivities();
  }, [selectedProyecto, actividadId, activitiesLoaded, getIdsProyectYAct]);
  ...
  function generarPDF() {
    ...
    const proyectoNombre = getProyectoName(selectedProyecto);
  const actividadNombre = getActividadName(selectedActividad);
  ...
  const getActividadName = (actividadId) => {
    const actividad = idsAct.find((actividad) => actividad._id === actividadId);
