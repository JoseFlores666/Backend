...
export const RegisterSolicitudPage = () => {
  ...
  const guardarDatos = () => {
    const { folio, fecha, suministro, pc, proyecto, actividad, justificacion, items } = formData;
    if (!folio || !fecha || !suministro || !pc || !proyecto || !actividad || !justificacion || items.length === 0) {
      alert('Por favor, complete todos los campos y agregue al menos un ítem.');
      return;
    }

    localStorage.setItem('datosSolicitud', JSON.stringify(formData));
    setFormData({
      folio: '',
      fecha: '',
      suministro: '',
  ...
  const agregarItem = () => {
    if (formData.items.length < 10) {
      setFormData({
        ...formData,
        items: [...formData.items, { cantidad: '', unidad: '', descripcion: '' }]
      });
    } else {
  ...
  return (
    <div className="body2">
      <div className="formulariodatos" id="formRef">
        ...
        <button type="button" onClick={guardarDatosYGenerarPDF}>Guardar cambios y generar PDF</button>
      ...
      <div className="botones">
        <button className="btn-primary" onClick={guardarDatos}>Guardar cambios</button>
      ...
      <div style={{ display: 'none' }}>
        ...
        <table id="miTabla" className="tabla">
          ...
          <tbody>
            <tr>
              <td id="td-proyecto" rowSpan="5">Proyecto: <br />{formData.proyecto}</td>
              <td></td>
              <td>{formData.items[0]?.cantidad}</td>
              <td></td>
              <td>{formData.items[0]?.unidad}</td>
              <td>{formData.items[0]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[1]?.cantidad}</td>
              <td></td>
              <td>{formData.items[1]?.unidad}</td>
              <td>{formData.items[1]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[2]?.cantidad}</td>
              <td></td>
              <td>{formData.items[2]?.unidad}</td>
              <td>{formData.items[2]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[3]?.cantidad}</td>
              <td></td>
              <td>{formData.items[3]?.unidad}</td>
              <td>{formData.items[3]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[4]?.cantidad}</td>
              <td></td>
              <td>{formData.items[4]?.unidad}</td>
              <td>{formData.items[4]?.descripcion}</td>
            </tr>
            <tr>
              <td id="td-actividad" rowSpan="5">Actividad: <br /> {formData.actividad}</td>
              <td></td>
              <td>{formData.items[5]?.cantidad}</td>
              <td></td>
              <td>{formData.items[5]?.unidad}</td>
              <td>{formData.items[5]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[6]?.cantidad}</td>
              <td></td>
              <td>{formData.items[6]?.unidad}</td>
              <td>{formData.items[6]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[7]?.cantidad}</td>
              <td></td>
              <td>{formData.items[7]?.unidad}</td>
              <td>{formData.items[7]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[8]?.cantidad}</td>
              <td></td>
              <td>{formData.items[8]?.unidad}</td>
              <td>{formData.items[8]?.descripcion}</td>
            </tr>
            <tr>
              <td></td>
              <td>{formData.items[9]?.cantidad}</td>
              <td></td>
              <td>{formData.items[9]?.unidad}</td>
              <td>{formData.items[9]?.descripcion}</td>
            </tr>
            <tr>
              <td rowSpan="">JUSTIFICACION PARA LA ADQUISICION:</td>
              <td id="td-justificacion" colSpan="6">{formData.justificacion}</td>
            </tr>
          </tbody>
        </table>
