...
export const RegisterSolicitudPage = () => {
  ...
  const handleProyectoChange = (value) => {
    ...
    // Utiliza la referencia formRef para acceder al formulario
    setSelectedProyectoNombre(formRef.current.querySelector("#Proyecto option:checked").innerText); // Actualizar el nombre del proyecto
  };
  ...
  const handleGeneratePDF = (event) => {
    ...
    const data = {
      ...
      pc: form.querySelector("#PC option:checked").innerText,
      proyecto: selectedProyectoNombre,
      actividad: form.querySelector("#Actividad option:checked").innerText,
  ...
  return (
    <div className="body2">
      ...
      <form className="formulariodatos" onSubmit={handleSubmit} ref={formRef}>
        ...
        <img src={Papel} id='image1' alt="Papel" style={{ height: '100%', width: '100%' }} onLoad={generarPDF} />
        <table id="miTabla" className="tabla">
          {/* Contenido de la tabla */}
        </table>
        <img src={Papel2} id='image2' alt="Papel2" style={{ height: '100%', width: '100%' }} onLoad={generarPDF} />
      </div>
      </form>
    </div>
  );
