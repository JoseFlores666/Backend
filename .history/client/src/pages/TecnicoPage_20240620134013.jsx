...
export const TecnicoPage = () => {
  const {
    ...
    getInfo,//consulta para traer el informe
    info,//una ves que se llame a getInfo info obtiene la consulta
  } = useSoli();
  ...
  const solicitudes = [
    {
      ...
      informe: {
        ...
        tipoDeTrabajo: "Eléctrico",
        tipoDeSolicitud: "Interna",
        descripcionDelServicio: "Revisión de cables",
      },
      solicitud: {
        insumosSolicitados: {
          fechaAtencion: "2024-06-15",
          cantidad: 10,
          descripcion: "Cables eléctricos",
          Observacionestecnicas: "Revisar estado de conectores",
        },
      },
      firmas: "123",
      user: "456",
    ...
    // Puedes añadir más solicitudes aquí
  ];
  ...
  useEffect(() => {
    const results = solicitudes.filter((solicitud) =>
      solicitud.folio.toLowerCase().includes(searchTerm.toLowerCase()) ||
      solicitud.informe.Solicita.nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
      solicitud.informe.Solicita.areaSolicitante.toLowerCase().includes(searchTerm.toLowerCase()) ||
      solicitud.informe.tipoDeMantenimiento.toLowerCase().includes(searchTerm.toLowerCase()) ||
      solicitud.informe.tipoDeTrabajo.toLowerCase().includes(searchTerm.toLowerCase()) ||
      solicitud.informe.tipoDeSolicitud.toLowerCase().includes(searchTerm.toLowerCase()) ||
