...
export const RegisterSolicitudPage = () => {
  ...
  const populateForm = (solicitud) => {
    ...
    try {
      ...
      // Seleccionar automáticamente el proyecto y la actividad de la solicitud
      const proyectoOption = form.querySelector(`#Proyecto option[value="${solicitud.proyecto}"]`);
      if (proyectoOption) {
        proyectoOption.selected = true;
      }
      const actividadOption = form.querySelector(`#Actividad option[value="${solicitud.actividad}"]`);
      if (actividadOption) {
        actividadOption.selected = true;
      }
    } catch (error) {
