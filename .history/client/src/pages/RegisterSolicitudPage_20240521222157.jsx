...
export const RegisterSolicitudPage = () => {
  ...
  const [pageTitle, setPageTitle] = useState("Registro");
  const [isUpdating, setIsUpdating] = useState(false); // Estado para controlar si se está actualizando
  const [solicitudToUpdate, setSolicitudToUpdate] = useState(null); // Estado para almacenar la solicitud a actualizar

  const { crearmySoli, getSoliById, updateSoli, getIdsProyect, ids, getIdsProyectYAct, idsAct = [] } = useSoli();
  const { user } = useAuth();

  useEffect(() => {
    if (solicitud) { // Si hay un ID, está en modo actualización
      setIsUpdating(true);
      getSoliById(id)
        .then((solicitud) => {
          setSolicitudToUpdate(solicitud);
          setSelectedDate(new Date(solicitud.fecha));
          setSelectedProyecto(solicitud.proyecto);
          setItems(solicitud.items);
          setPageTitle("Actualizar"); // Cambia el título a "Actualizar"
        })
        .catch((error) => {
          console.error("Error fetching solicitud:", error);
        });
    }
  }, [solicitud, getSoliById]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (isUpdating && solicitudToUpdate) {
      await updateSoli(solicitudToUpdate._id, data);
    } else {
  ...
  return (
    <div className="body2">
      <form className="formulariodatos" onSubmit={handleSubmit} ref={formRef}>
        <h1 className="titulo">{pageTitle}</h1>
