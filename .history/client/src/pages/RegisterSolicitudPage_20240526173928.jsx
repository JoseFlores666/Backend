...
export const RegisterSolicitudPage = () => {
  ...
  const { crearmySoli, getIdsProyectYAct, getIdsProyect , idsAct = [] } = useSoli();
  const { user } = useAuth();
  const id = user.id;
  ...
  return (
    <div className="body2">
      <div className="formulariodatos" id="formRef">
        ...
        <div className="division">
        <label className="labels">Proyecto:</label>
        {/* Seleccionar el proyecto del estado */}
        <select
          id="Proyecto"
          value={datos.proyecto}
          onChange={(e) => setDatos({ ...datos, proyecto: e.target.value })}
        >
          <option value="">Seleccione el Proyecto</option>
          {/* Mapear los proyectos en opciones del select */}
