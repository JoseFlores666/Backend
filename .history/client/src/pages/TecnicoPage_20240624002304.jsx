...
export const TecnicoPage = () => {
  ...
  const openModal = (imageSrcArray, index) => {
    setModalImages(imageSrcArray);
    ...
    <div className="overflow-x-auto">
      ...
      <table className="w-full min-w-full divide-y divide-white-200 text-sm text-black rounded-lg overflow-hidden">
        <thead className="bg-black text-white">
          <tr>
            ...
            <th className="px-3 py-1 text-left font-medium uppercase tracking-wider border text-center w-1/12">Imagen</th>
          ...
          {currentSolicitudes.map((solicitud, index) => (
            ...
            >
              ...
              <td className="px-3 py-2 whitespace-normal break-words border text-center">
                {solicitud.solicitud.insumosSolicitados.imagen.length > 0 ? (
                  <div className="grid grid-cols-2 gap-1">
                    {solicitud.solicitud.insumosSolicitados.imagen.map((image, imgIndex) => (
                      <LazyLoadImage
                        key={imgIndex}
                        src={image}
                        alt={`Preview ${imgIndex + 1}`}
                        className="w-10 h-10 object-cover rounded cursor-pointer"
                        onClick={() => openModal(solicitud.solicitud.insumosSolicitados.imagen, imgIndex)}
                      />
                    ))}
                  </div>
                ) : (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50" onClick={handleClickOutside}>
          <div ref={modalContentRef} className="bg-white p-4 rounded-lg shadow-lg relative">
            <button
              className="absolute top-2 right-2 text-3xl text-black hover:text-red-700 focus:outline-none"
              onClick={closeModal}
            >
            ...
            <div className="">
              ...
              {modalImages.length > 1 && (
                <>
                  <button
                    className="absolute top-1/2 left-4 transform -translate-y-1/2 text-2xl text-black hover:text-blue-600 focus:outline-none"
                    onClick={() =>
                  ...
                  <button
                    className="absolute top-1/2 right-4 transform -translate-y-1/2 text-2xl text-black hover:text-blue-600 focus:outline-none"
                    onClick={() =>
                      setCurrentImageIndex((prevIndex) =>
