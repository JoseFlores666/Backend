...
export const RegisterSolicitudPage = () => {
  ...
  // Estado para controlar si los proyectos se han cargado
...
const handleProyectoChange = (event) => {
  ...
  // Al seleccionar un proyecto, cargar las actividades correspondientes
  getIdsProyectYAct(event.target.value)
    .then((data) => {
      setIdsAct(data); // Usar setIdsAct para actualizar el estado de idsAct
    })
    .catch((error) => {
      console.error("Error fetching activities:", error);
    });
};
Con esta modificación, estás utilizando el estado idsAct definido con useState en lugar de declarar una nueva variable constante. Esto debería resolver el problema de la declaración duplicada y eliminar el error "Assignment to constant variable".
  ...
  const handleProyectoChange = (event) => {
    ...
    getIdsProyectYAct(event.target.value)
      .then((data) => {
        idsAct = data; // Suponiendo que getIdsProyectYAct devuelve las actividades
      })
      .catch((error) => {
        console.error("Error fetching activities:", error);
      });
