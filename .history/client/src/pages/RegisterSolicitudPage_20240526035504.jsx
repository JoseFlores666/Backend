    ...
    itemsChunks.forEach((chunk, pageIndex) => {
      ...
      doc.autoTable({
        ...
        body: chunk.map((item, index) => {
          if (index === 0) {
            return [
              { content: `Proyecto: ${getProyectoName(selectedProyecto)}`, rowSpan: 2 }, // Primera fila de la primera columna
              "", "", "", "", "" // Segunda fila de la primera columna y celdas vacías en las demás columnas
            ];
          } else {
            return [
              "", "", item.cantidad, item.unidad, item.descripcion, "" // Celdas vacías en la primera columna y datos en las demás columnas
            ];
          }
        }),
