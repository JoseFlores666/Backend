...
export const RegisterSolicitudPage = () => {
  ...
  const [solicitudLoaaded, setUnaSolicitudLoaaded] = useState(false);
  ...
  useEffect(() => {
    const fetchSolicitudData = async () => {
      if (solicitudId && !solicitudLoaaded) { // Corrected typo here
        setIsEditing(false);
        setUnaSolicitudLoaaded(true);
        try {
          await getunSolitud(solicitudId);
          if (unasoli) {
            setSolicitud(unasoli);
            console.log("Hola esta es tu soli"unasoli)
    ...
    fetchSolicitudData();
  }, [solicitudId, unasoli, solicitudLoaaded]); // Make sure to add solicitudLoaded to the dependency array
