...
export const RegisterTecnicoPage = () => {
  ...
  const { createInfo, getIdsProyect, ids, getIdsProyectYAct, traeFolioInternoInforme, myFolioInterno } = useSoli();
  ...
  const fetchFolio = async () => {
    try {
      ...
      setValue("folio", myFolioInterno);
    } catch (error) {
      console.error("Error al obtener la el folio:", error);
    }
  };
  useEffect(() => {
    // After fetching folio, set the value in the form
    if (myFolioInterno) {
      setValue("folio", myFolioInterno);
    }
  }, [myFolioInterno, setValue]);
      ...
      <form ref={formRef} method="post" target="_blank" onSubmit={handleSubmit(onSubmit)}>
        <div className="bg-white p-6 rounded-md shadow-md">
          ...
          <div class="mb-6 text-black">
            <label class="block text-sm font-medium mb-1">Descripción (servicio requerido):</label>
            <textarea
              class="w-full resize-none p-3 border border-gray-400 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
              id="desc"
              name="desc"
              {...register("desc")}
            ></textarea>
            {errors.desc && (
              <p class="text-red-500 mt-0.5">{errors.desc.message}</p>
            )}
          </div>
