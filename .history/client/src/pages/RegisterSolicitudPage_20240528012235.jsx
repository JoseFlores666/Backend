  ...
  const guardarDatos = () => {
    ...
    const fetchProyecto = async () => {
      try {
        ...
        // Llamar a setFetchPDFFlag(true) aquí para activar la generación de PDF
  ...
  useEffect(() => {
    const activarGenerarPDF = async () => {
      if (proyecto && actividad) {
        try {
          console.log("Obteniendo información del proyecto y la actividad...");
          await getIdsProyectYActIndividual(proyecto, actividad);
          setActivitiesLoaded(true);
          generarPDF();
        } catch (error) {
          console.error("Error al obtener información del proyecto y la actividad:", error);
        }
      } else {
