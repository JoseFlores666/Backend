...
export const RegisterSolicitudPage = () => {
  ...
  const [idReceived, setIdReceived] = useState(false); // Estado local para controlar si se ha recibido el ID
  ...
  useEffect(() => {
    if (data && !idReceived) { // Solo ejecutar si hay data y no se ha recibido el ID antes
      setIsEditing(true);
      getunSolitud(data)
        .then((solicitudData) => {
          setSolicitud(solicitudData);
          populateForm(solicitudData);
          setIdReceived(true); // Marcar que se ha recibido el ID
        })
        .catch((error) => {
  ...
  }, [data, idReceived]); // Asegúrate de incluir idReceived en las dependencias del efecto
  ...
  const populateForm = (solicitudData) => {
    const form = formRef.current;
    if (!form) return;
    
    form.querySelector("#Folio").value = solicitudData.folio;
    form.querySelector("#Suministro").value = solicitudData.tipoSuministro;
    form.querySelector("#PC").value = solicitudData.procesoClave;
    setSelectedProyecto(solicitudData.proyecto);
    setSelectedDate(new Date(solicitudData.fecha));
    setItems(solicitudData.items.map((item) => ({
      cantidad: item.cantidad,
      unidad: item.unidad,
      descripcion: item.descripcion,
    })));
    form.querySelector("#Justificacion").value = solicitudData.justificacion;
  };
