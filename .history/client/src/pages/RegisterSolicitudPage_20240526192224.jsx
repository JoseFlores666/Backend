...
export const RegisterSolicitudPage = () => {
  ...
  const handleProyectoChange = (proyecto) => {
    setSelectedProyecto(proyecto); // Actualiza el proyecto seleccionado
    setActividad(""); // Limpia la actividad seleccionada
  };
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      if (SelectedProyecto && !activitiesLoaded) {
        try {
          await getIdsProyectYAct(SelectedProyecto); // Obtén las actividades del proyecto seleccionado
...
>
  ...
  {ids.map((proyecto) => (
    <option key={proyecto.id} value={proyecto}>{/* Utiliza el objeto completo del proyecto */}
      {proyecto.nombre}
    </option>
  ))}
</select>
