...
export const RegisterSolicitudPage = () => {
  ...
  const [solicitud, setSolicitud] = useState(null); // Estado para almacenar la solicitud
  ...
  useEffect(() => {
    if (solicitudId) {
      setIsEditing(true);
      // Obtener la solicitud por el ID solo una vez cuando solicitudId cambie
      getunSolitud(solicitudId)
        .then((solicitudData) => {
          if (solicitudData) {
            setSolicitud(solicitudData);
            // Rellenar el formulario con la solicitud obtenida
  ...
  const handleSubmit = (event) => {
    ...
    if (isEditing && solicitud) {
      updateSoli(solicitud._id, data).then(() => {
  ...
  const populateForm = (solicitudData) => {
    ...
    try {
      ...
      setSelectedDate(new Date(solicitudData.fecha || Date.now()));
