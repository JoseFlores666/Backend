...
export const RegisterSolicitudPage = () => {
  ...
  const miFuncionPrincipal = async () => {
    ...
    try {
      const response = await getIdsProyectYActIndividual(idProyecto, idActividad);
      if (response.proyect && response.unProyectAct) {
        setProyectoNombre(response.proyect.nombre);
        setActividadNombre(response.unProyectAct.nombre);
      } else {
        console.error("Error: Proyecto o Actividad no encontrados");
      }
