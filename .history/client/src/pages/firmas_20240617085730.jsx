...
export const Firmas = () => {
  const {
    ...
    resolver: zodResolver(firmasSchema), // Resuelve la validación usando el esquema firmasSchema
  });

  const { editarFirmas } = useSoli(); // Obtiene la función editarFirmas del contexto

  const guardarDatos = async (data) => {
    console.log("Datos guardados:", data);

    const datosActualizados = {
      solicitud: data.solicitante, // Aquí deberías ajustar según cómo se llamen los campos en tu formulario
      revision: data.jefeInmediato,
      validacion: data.direccion,
      autorizacion: data.rectoria,
    };

    try {
      await editarFirmas(datosActualizados); // Llama a la función editarFirmas del contexto con datosActualizados
