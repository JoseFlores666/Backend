...
export const RegisterSolicitudPage = () => {
  ...
  const [fetchActivitiesFlag, setActivitiesLoaded] = useState(false);
  const {
    crearmySoli,
    getIdsProyect,
  ...
  useEffect(() => {
    ...
    // Solo realiza la consulta de actividades si el proyecto cambia y si la bandera fetchActivitiesFlag está activada
    if (proyecto && fetchActivitiesFlag) {
      fetchActivities();
      setFetchActivitiesFlag(false); // Desactiva la bandera después de realizar la consulta
    }
