...
export const RegisterSolicitudPage = () => {
  ...
  useEffect(() => {
    if (fetchPDFFlag) {
      generarPDF({ proyecto, actividad });
      setFetchPDFFlag(false); // Restablecer la bandera después de generar el PDF
    }
  }, [fetchPDFFlag, proyecto, actividad]);
  ...
  useEffect(() => {
    const fetchIndividualProyectAndActivity = async () => {
      try {
        const response = await getIdsProyectYActIndividual(proyecto, actividad);
        console.log('Proyectos y actividades obtenidos:', response);
        setActivitiesLoaded(true);
        generarPDF({ proyecto, actividad }); // Generar PDF con los nuevos datos
      } catch (error) {
        console.error("Error fetching individual project and activity:", error);
      }
    };
  
    fetchIndividualProyectAndActivity();
  }, [proyecto, actividad, getIdsProyectYActIndividual]);
  
  
  function generarPDF({ proyecto, actividad }) {
    ...
    <div className="body2">
      <div className="formulariodatos" id="formRef">
        ...
        <div className="division">
          <select
            id="Suministro"
            value={suministro} className="Inputs"
          ...
          <select id="PC" value={pc}className="Inputs" onChange={(e) => setPc(e.target.value)}>
        ...
        <div className="division">
          <select
            id="Proyecto"
            value={proyecto} className="Inputs"
            onChange={(e) => {
          ...
          <select
            id="Actividad"
            value={actividad} className="Inputs"
