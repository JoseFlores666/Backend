...
export const RegisterSolicitudPage = () => {
  ...
  const { crearmySoli,getIdsProyect } = useSoli();
 
  const ids = getIdsProyect();
  console.log(ids)
  const { user } = useAuth();
  ...
  return (
    <div className="body2">
      <form className="formulariodatos" onSubmit={handleSubmit} ref={formRef}>
        ...
        <div className="division">
        <select id="Proyecto" name="Proyecto" className="select" defaultValue="">
...
<button type="button" onClick={addItem} className="btn-agregarfila btn-primary">
  Agregar otra fila
        ...
        <textarea
          className="inputs4"
          id="Justificacion"
          name="Justificacion"  placeholder="Ingrese la justificación de la solicitudes"
        ></textarea>
